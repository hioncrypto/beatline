        ...
        try:
            ...
            if edge:
                ...
                if not confirmed:
                    ...
                    should_push = False
                # Buy first — push I/O to phones must not delay the fill.
                if confirmed:
                    try:
                        side = edge["side"]
                        if side == "above":
...
def try_server_auto_trade(
    *,
    ...
    try:
        return _try_server_auto_trade_body(
            ticker=ticker,
            side=side,
            ask_cents=ask_cents,
            suggest_stake=suggest_stake,
            bid_cents=bid_cents,
            opposite_bid_cents=opposite_bid_cents,
            creds=creds,
            key=key,
            finish=finish,
        )
...
def _try_server_auto_trade_body(
    *,
    ticker: str,
    side: str,
    ask_cents,
